...
def read_matlab_matrix(mat_filename, variable_name):
    matlab_matrix = numpy.ma.masked_invalid(sio.loadmat(mat_filename)[variable_name])
    numpy.ma.set_fill_value(matlab_matrix, numpy.nan)
...
def matlab_matrix_to_land_tile_geotiff(matlab_filename, matlab_variable_name, tiff_filename, nodata_value=DEFAULT_NODATA_VALUE):
    ...
    matrix[numpy.isnan(matrix)] = nodata_value
    matrix_to_land_tile_geotiff(matrix, x_min, y_max, tiff_filename)

def matlab_to_global_tiff(matlab_directory, variable_name, output_directory, output_filename, nodata_value=None):
    ...
    # subprocess.call(['gdal_merge', '-n', nodata_value, '-a_nodata', nodata_value, '-o', output_filename + tiff_file_list)

    merge_geotiffs(os.path.join(output_directory, '*.tif'), output_filename, nodata_value)